...
# Read data
getwd()
load("ICD_Data_for_generating_embedding.Rdata") # I named the object as data_merged in the summary R script

# Prepare data
# Note: need to figure out a way to define "Month"
data_for_pmisvd = data_merged%>%
  group_by(Month = discharge_year_month, Patient = subject_id, Parent_Code = icd_code)%>%
  summarize(Count = n())
...
# SVD embeddings
m_icd_embeds_50 = get_svd(m_pmi, svd_rank = 50) 
m_icd_embeds_100 = get_svd(m_pmi, svd_rank = 100)
m_icd_embeds_150 = get_svd(m_pmi, svd_rank = 150)

save(m_icd_embeds_50,file = "ICD_50dim.Rdata")
save(m_icd_embeds_100,file = "ICD_100dim.Rdata")
save(m_icd_embeds_150,file = "ICD_150dim.Rdata")
