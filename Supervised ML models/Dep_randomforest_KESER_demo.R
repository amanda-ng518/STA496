...
process_test_data <- function(data) {
  ...
  # Create an indicator variable for each feature code
  for (feature in feature_codes) {
...
rf_model <- randomForest(dep ~ . -subject_id,ntree = 100, data = LR_train, type = classification,na.action = na.omit)
...
bootstrap_metrics_by_single_group <- function(test_data, num_bootstrap, demographic_col) {
  ...
  for (i in 1:num_bootstrap) {
    ...
    subgroup_results <- lapply(demographic_groups, function(group_data) {
      ...
      # Make predictions
      predictions <- predict(rf_model, newdata = group_data)
      prob <- as.numeric(predict(rf_model, group_data, type = "prob")[,2])      
      ...
      # Compute AUC
      roc_curve <- roc(group_data$dep, prob, quiet = TRUE)
      auc_value <- auc(roc_curve)
...
bootstrap_metrics_by_group <- function(test_data, num_bootstrap, demographic_col) {
  ...
  mean_diff <- colMeans(diff_results_df)
  se_diff_results <- colSums((diff_results_df - mean_diff)^2) / num_bootstrap  # Variance formula, sqrt(variance) = SE
